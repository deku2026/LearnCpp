# 第 6 部分 · 支线 F · 内存模型与并发 (深水原子/无锁)

- 路线图: `第6部分-支线F-内存模型与并发.md`
...
```bash
ls src/deep_dive_memory_model_and_concurrency
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_memory_model_and_concurrency__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_memory_model_and_concurrency__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_memory_model_and_concurrency__<topic>, <new_item>)` 里的 `<new_item>`.
