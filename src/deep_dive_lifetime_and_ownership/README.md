# 第 6 部分 · 支线 B · 生命周期与所有权 + GSL

- 路线图: `第6部分-支线B-生命周期与所有权.md`
...
```bash
ls src/deep_dive_lifetime_and_ownership
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_lifetime_and_ownership__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_lifetime_and_ownership__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_lifetime_and_ownership__<topic>, <new_item>)` 里的 `<new_item>`.
