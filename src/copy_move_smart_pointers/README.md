...
```bash
ls src/copy_move_smart_pointers
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'copy_move_smart_pointers__'`;
...
ctest --preset windows-debug --output-on-failure -R 'copy_move_smart_pointers__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(copy_move_smart_pointers__<topic>, <new_item>)` 里的 `<new_item>`.
