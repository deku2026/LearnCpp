# 第 6 部分 · 支线 G · ABI / 编译 / 链接 / UB / 性能

- 路线图: `第6部分-支线G-ABI编译链接UB性能.md`
...
```bash
ls src/deep_dive_abi_compile_link_ub_performance
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_abi_compile_link_ub_performance__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_abi_compile_link_ub_performance__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_abi_compile_link_ub_performance__<topic>, <new_item>)` 里的 `<new_item>`.
