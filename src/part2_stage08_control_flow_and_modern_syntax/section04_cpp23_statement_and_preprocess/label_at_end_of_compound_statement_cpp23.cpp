// LearnCpp placeholder
// Doc     : part2-stage08-control-flow-and-modern-syntax.md
// Stage   : part2_stage08_control_flow_and_modern_syntax
// Section : section04_cpp23_statement_and_preprocess
// Item    : label_at_end_of_compound_statement_cpp23
//
// TODO: read cppreference, sketch a minimal example, check godbolt / C++ Insights,
//       then replace GTEST_SKIP with real EXPECT_* / ASSERT_* assertions.

#include <gtest/gtest.h>

namespace learn_cpp::part2_stage08::section04 {

TEST(part2_stage08_section04, label_at_end_of_compound_statement_cpp23) {
    GTEST_SKIP() << "todo: label_at_end_of_compound_statement_cpp23";
}

}  // namespace learn_cpp::part2_stage08::section04
