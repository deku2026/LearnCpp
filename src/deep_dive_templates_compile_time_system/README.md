# 第 6 部分 · 支线 E · 模板与编译期系统 (深水 TMP)

- 路线图: `第6部分-支线E-模板与编译期系统.md`
...
```bash
ls src/deep_dive_templates_compile_time_system
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_templates_compile_time_system__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_templates_compile_time_system__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_templates_compile_time_system__<topic>, <new_item>)` 里的 `<new_item>`.
