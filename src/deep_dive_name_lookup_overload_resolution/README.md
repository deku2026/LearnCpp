# 第 6 部分 · 支线 D · 名字查找 / ADL / 重载决议

- 路线图: `第6部分-支线D-名字查找ADL与重载决议.md`
...
```bash
ls src/deep_dive_name_lookup_overload_resolution
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_name_lookup_overload_resolution__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_name_lookup_overload_resolution__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_name_lookup_overload_resolution__<topic>, <new_item>)` 里的 `<new_item>`.
