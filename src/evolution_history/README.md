...
C++ 每代头条特性 + 被废弃 / 移除 / 不该再用的旧惯用法及现代替代品.

## 主题子目录

```bash
ls src/evolution_history
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'evolution_history__'`;
...
ctest --preset windows-debug --output-on-failure -R 'evolution_history__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(evolution_history__<topic>, <new_item>)` 里的 `<new_item>`.
