// LearnCpp placeholder
// Doc     : part2-stage04-classes-and-object-model-basics.md
// Stage   : part2_stage04_classes_and_object_model_basics
// Section : section05_operator_overloading
// Item    : comparison_operators_cpp20_spaceship
//
// TODO: read cppreference, sketch a minimal example, check godbolt / C++ Insights,
//       then replace GTEST_SKIP with real EXPECT_* / ASSERT_* assertions.

#include <gtest/gtest.h>

namespace learn_cpp::part2_stage04::section05 {

TEST(part2_stage04_section05, comparison_operators_cpp20_spaceship) {
    GTEST_SKIP() << "todo: comparison_operators_cpp20_spaceship";
}

}  // namespace learn_cpp::part2_stage04::section05
