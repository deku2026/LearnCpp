# 阶段 14 · 特性与关键字全表 / 逐标准对账

- 路线图: `第2部分-阶段14-特性与关键字全表.md`
...
和 cppreference 'C++23 compiler support' + 'Feature testing' 两张表逐行打勾, 把 C++11 -> C++23 所有特性收口.

## 主题子目录

```bash
ls src/keywords_and_features_index
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'keywords_and_features_index__'`;
...
ctest --preset windows-debug --output-on-failure -R 'keywords_and_features_index__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(keywords_and_features_index__<topic>, <new_item>)` 里的 `<new_item>`.
