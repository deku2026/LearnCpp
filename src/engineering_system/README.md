# 第 4 部分 · 工程系统 (CMake / 包 / 测试 / Sanitizer / CI)
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'engineering_system__'`;
...
ctest --preset windows-debug --output-on-failure -R 'engineering_system__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(engineering_system__<topic>, <new_item>)` 里的 `<new_item>`.
