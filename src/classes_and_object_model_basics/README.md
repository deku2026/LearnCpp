# 阶段 4 · 类与对象模型基础 (含 deducing this)

- 路线图: `第2部分-阶段4-类与对象模型基础.md`
...
```bash
ls src/classes_and_object_model_basics
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'classes_and_object_model_basics__'`;
...
ctest --preset windows-debug --output-on-failure -R 'classes_and_object_model_basics__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(classes_and_object_model_basics__<topic>, <new_item>)` 里的 `<new_item>`.
