...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'templates_and_generics__'`;
...
ctest --preset windows-debug --output-on-failure -R 'templates_and_generics__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(templates_and_generics__<topic>, <new_item>)` 里的 `<new_item>`.
