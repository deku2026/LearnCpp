...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'deep_dive_object_model__'`;
...
ctest --preset windows-debug --output-on-failure -R 'deep_dive_object_model__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(deep_dive_object_model__<topic>, <new_item>)` 里的 `<new_item>`.
