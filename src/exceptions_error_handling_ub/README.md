# 阶段 9 · 异常 / 错误处理 / UB 纪律 (含 std::expected)

- 路线图: `第2部分-阶段9-异常-错误处理与UB纪律.md`
...
```bash
ls src/exceptions_error_handling_ub
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'exceptions_error_handling_ub__'`;
...
ctest --preset windows-debug --output-on-failure -R 'exceptions_error_handling_ub__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(exceptions_error_handling_ub__<topic>, <new_item>)` 里的 `<new_item>`.
