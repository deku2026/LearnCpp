# 阶段 12 · 协程 (C++20 机制 + C++23 std::generator)
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'coroutines__'`;
...
ctest --preset windows-debug --output-on-failure -R 'coroutines__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(coroutines__<topic>, <new_item>)` 里的 `<new_item>`.
