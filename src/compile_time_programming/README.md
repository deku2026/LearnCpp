# 阶段 7 · 编译期编程 (constexpr / consteval / constinit / if consteval)

- 路线图: `第2部分-阶段7-编译期编程.md`
...
```bash
ls src/compile_time_programming
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'compile_time_programming__'`;
...
ctest --preset windows-debug --output-on-failure -R 'compile_time_programming__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(compile_time_programming__<topic>, <new_item>)` 里的 `<new_item>`.
