# 阶段 11 · 并发与多线程 (线程 / 同步 / 原子 / 内存模型)

- 路线图: `第2部分-阶段11-并发与多线程.md`
...
```bash
ls src/concurrency_and_threading
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'concurrency_and_threading__'`;
...
ctest --preset windows-debug --output-on-failure -R 'concurrency_and_threading__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(concurrency_and_threading__<topic>, <new_item>)` 里的 `<new_item>`.
