...
```bash
ls src/control_flow_and_modern_syntax
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'control_flow_and_modern_syntax__'`;
...
ctest --preset windows-debug --output-on-failure -R 'control_flow_and_modern_syntax__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(control_flow_and_modern_syntax__<topic>, <new_item>)` 里的 `<new_item>`.
