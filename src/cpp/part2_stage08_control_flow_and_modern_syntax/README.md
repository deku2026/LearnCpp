# Part 2 / Stage 08 · 控制流与现代语法增强

- 路线图源文档: `第2部分-阶段8-控制流与现代语法增强.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

控制流收口 + C++17/20/23 在语句/属性层的现代增强 (range-for 临时延长 / [[assume]] / #elifdef 等).

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part2_stage08_control_flow_and_modern_syntax
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage08/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage08/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage08/section01/<item_slug>
```
