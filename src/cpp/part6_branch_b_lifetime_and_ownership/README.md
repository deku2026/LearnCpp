# Part 6 · Branch B · 生命周期与所有权 + GSL

- 路线图源文档: `第6部分-支线B-生命周期与所有权.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

对象生命周期 / 临时对象延长 (含 C++23 P2718) / 悬垂的所有形态 / 所有权类型化 (owner<>/not_null<>) / C++23 临时引用 traits.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part6_branch_b_lifetime_and_ownership
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part6/b/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part6/b/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part6/b/section01/<item_slug>
```
