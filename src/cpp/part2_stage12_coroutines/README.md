# Part 2 / Stage 12 · 协程 (C++20 机制 + C++23 std::generator)

- 路线图源文档: `第2部分-阶段12-协程.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

吃透 C++20 协程: 编译器把它改写成什么 (协程帧 / promise / awaiter / coroutine_handle), 并用 C++23 std::generator 做对照.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part2_stage12_coroutines
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage12/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage12/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage12/section01/<item_slug>
```
