...
//
// Covers: handles as scheduling tokens
...
void demo_intermediate() {
    ...
    int a = 0;
    int b = 0;
    auto r1 = [](int* p) -> Resumable {
        *p = 1;
        co_return;
    }(&a);
    auto r2 = [](int* p) -> Resumable {
        *p = 2;
        co_return;
    }(&b);
    std::vector<std::coroutine_handle<>> ready{r1.h, r2.h};
    for (auto h : ready) {
        h.resume();
    }
    assert(a == 1 && b == 2);
...
void demo_expert() {
#if defined(__cpp_impl_coroutine) || defined(__cpp_coroutines)
    // A scheduler stores handles and resumes them later — cooperative multitasking.
    int n = 0;
    auto r = [](int* p) -> Resumable {
        co_await std::suspend_always{};
        *p = 9;
