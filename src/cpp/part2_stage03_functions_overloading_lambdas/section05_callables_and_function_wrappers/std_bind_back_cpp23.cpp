...
int run(int /*argc*/, char** /*argv*/) {
    ...
    // §入门：固定最后一个参数
    // -------------------------------------------------------------------------
    [[maybe_unused]] auto sub_from = std::bind_back(sub, 3);  // sub_from(a) == sub(a, 3)
    ...
    // -------------------------------------------------------------------------
    [[maybe_unused]] auto plus_c = std::bind_back(sum3, 100);  // (a,b) -> a+b+100
    assert(plus_c(1, 2) == 103);

    [[maybe_unused]] auto bracket = std::bind_back(wrap, '[', ']');
    assert(bracket("x") == "[x]");

    // pow：bind_back(pow, 2) → square(base) = pow(base, 2)
    [[maybe_unused]] auto square = std::bind_back(static_cast<double (*)(double, double)>(std::pow), 2.0);
    ...
    // §专家：lambda 等价、管道场景
    // -------------------------------------------------------------------------
    [[maybe_unused]] auto sub_from_lam = [](int a) { return sub(a, 3); };
    assert(sub_from_lam(10) == sub_from(10));

    // 范围适配器 / 部分应用尾部选项时常 bind_back；仍优先可读 lambda。
    [[maybe_unused]] auto greet =
        std::bind_back([](const std::string& name, const std::string& mark) { return name + mark; }, "!");
