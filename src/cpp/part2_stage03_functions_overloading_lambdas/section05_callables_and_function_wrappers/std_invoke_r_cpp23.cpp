...
int run(int /*argc*/, char** /*argv*/) {
    ...
    [[maybe_unused]] const int n = std::invoke_r<int>(scale, 4);  // 6.0 → 6
    assert(n == 6);
    ...
    [[maybe_unused]] const int same = std::invoke_r<int>(Gen{}, 3);
    assert(same == 30);

    [[maybe_unused]] const double d = std::invoke_r<double>([](int x) { return x; }, 5);
    assert(d == 5.0);
    ...
    [[maybe_unused]] auto pipeline = [](auto&& f, auto&&... args) {
