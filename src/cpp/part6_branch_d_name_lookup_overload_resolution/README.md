# Part 6 · Branch D · 名字查找 / ADL / 重载决议

- 路线图源文档: `第6部分-支线D-名字查找ADL与重载决议.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

C++ '玄学报错' 的总根源: 名字查找 / 依赖名 / 两阶段查找 / 重载决议 / 偏序 / CPO / range_adaptor_closure / tag_invoke.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part6_branch_d_name_lookup_overload_resolution
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part6/d/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part6/d/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part6/d/section01/<item_slug>
```
