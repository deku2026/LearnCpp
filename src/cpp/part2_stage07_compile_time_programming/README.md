# Part 2 / Stage 07 · 编译期编程 (constexpr / consteval / constinit / if consteval)

- 路线图源文档: `第2部分-阶段7-编译期编程.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

把计算搬到编译期; C++23 进一步放宽 constexpr, 引入 if consteval / constexpr unique_ptr / constexpr to_chars.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part2_stage07_compile_time_programming
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage07/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage07/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage07/section01/<item_slug>
```
