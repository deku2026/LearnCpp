# Part 6 · Branch G · ABI / 编译 / 链接 / UB / 性能

- 路线图源文档: `第6部分-支线G-ABI编译链接UB性能.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

ABI / name mangling / 符号导出 / ODR / 静态 vs 动态库 / LTO / UB 全谱 / pointer provenance / implicit object creation / start_lifetime_as / launder / 类型双关 / 优化开关.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part6_branch_g_abi_compile_link_ub_performance
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part6/g/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part6/g/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part6/g/section01/<item_slug>
```
...
CMake `file(GLOB_RECURSE CONFIGURE_DEPENDS "src/cpp/*.cpp")` 在下次 `cmake --build` 自动接上, 不用动 CMakeLists.
