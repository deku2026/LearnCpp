# Part 2 / Stage 05 · 拷贝与移动语义 / 智能指针

- 路线图源文档: `第2部分-阶段5-拷贝与移动语义-智能指针.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

吃透移动语义 + 完美转发 + 智能指针 - 现代 C++ 性能与安全的命门; 和旧 C++/Qt 隐式共享分叉最大.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part2_stage05_copy_move_smart_pointers
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage05/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage05/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage05/section01/<item_slug>
```
