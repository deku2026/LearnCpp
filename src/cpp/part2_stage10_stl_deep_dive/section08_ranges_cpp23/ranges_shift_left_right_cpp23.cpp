...
int run(int /*argc*/, char** /*argv*/) {
    ...
    [[maybe_unused]] auto left_end = std::shift_left(v.begin(), v.end(), 2);
    // 有效前缀 [3,4,5]; 末 2 未指定
    assert(std::ranges::equal(std::ranges::subrange(v.begin(), left_end), std::vector{3, 4, 5}));
    assert(v[0] == 3 && v[1] == 4 && v[2] == 5);

    std::vector<int> w{1, 2, 3, 4, 5};
    [[maybe_unused]] auto right_begin = std::shift_right(w.begin(), w.end(), 2);
    // 有效后缀 [1,2,3]
    ...
    [[maybe_unused]] auto r0 = std::shift_left(a.begin(), a.end(), 0);
    assert(std::ranges::equal(std::ranges::subrange(a.begin(), r0), a));
    std::vector<int> b{1, 2, 3};
    [[maybe_unused]] auto rbig = std::shift_left(b.begin(), b.end(), 10);
