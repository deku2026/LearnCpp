# Part 2 / Stage 04 · 类与对象模型基础 (含 deducing this)

- 路线图源文档: `第2部分-阶段4-类与对象模型基础.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

掌握类机制: RAII / 特殊成员函数的 Rule of 0/3/5 联动 / C++23 头条特性 deducing this (合并 const-非const-左-右四份重载, 简化 CRTP).

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part2_stage04_classes_and_object_model_basics
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage04/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage04/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage04/section01/<item_slug>
```
