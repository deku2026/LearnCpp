...
- 路线图源文档: `第3部分-标准库系统化.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
- 主路线索引: `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/Cpp-Modern-完整学习路线图-C++23.md`

## 目标

把标准库按 '知道有什么 / 知道坑在哪 / 知道复杂度' 系统化收口; 配合 cppreference 'Standard library header' 索引打勾.

## 子目录 (sectionNN_xxx, 按文档大项顺序)

```bash
ls src/cpp/part3_standard_library_systematized
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part3/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part3/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part3/section01/<item_slug>
```
