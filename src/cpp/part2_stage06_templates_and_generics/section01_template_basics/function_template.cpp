...
void demo_basics() {
    assert(my_max(3, 5) == 5);
    assert(my_max(2.5, 1.5) == 2.5);
    assert(my_max<std::string>("a", "b") == "b");
}

void demo_intermediate() {
    assert(add_mixed(3, 2.5) == 5.5);
...
void demo_expert() {
    ...
    assert(my_max<int>(3, 5) == 5);
    assert(my_max<double>(3, 5) == 5.0);

    const int a = 10;
    const int b = 20;
    assert(my_max(a, b) == 20);

    auto by_value = [](auto x, auto y) { return my_max(x, y); };
    assert(by_value(std::string{"zz"}, std::string{"aa"}) == "zz");
}
