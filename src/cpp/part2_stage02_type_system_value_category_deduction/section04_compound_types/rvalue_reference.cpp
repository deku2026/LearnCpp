...
void demo_intermediate() {
    int x = 10;
    LEARN_CHECK(classify(x) == 1);             // lvalue
    LEARN_CHECK(classify(20) == 2);            // prvalue
    LEARN_CHECK(classify(std::move(x)) == 2);  // xvalue via move

    std::string src = "payload";
    std::string dst = std::move(src);
    LEARN_CHECK(dst == "payload");
...
void demo_expert() {
    ...
    int&& rr = 5;
    LEARN_CHECK(classify(rr) == 1);             // named => lvalue
    LEARN_CHECK(classify(std::move(rr)) == 2);  // cast back to rvalue

    // Move construction of containers
    std::string a = "abcdefgh";
    std::string b(std::move(a));
    LEARN_CHECK(b == "abcdefgh");
