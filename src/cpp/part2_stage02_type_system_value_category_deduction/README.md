# Part 2 / Stage 02 · 类型系统 / 值类别 / 类型推导

- 路线图源文档: `第2部分-阶段2-类型系统-值类别与类型推导.md` (在 `C:/MyFile/ArcForges/ArchitectureDesign/CppStudy/` 下找原始中文版)
...
```bash
ls src/cpp/part2_stage02_type_system_value_category_deduction
```
...
4. 跑这一个 topic:
   ```
   build/windows-debug/bin/learn_cpp.exe part2/stage02/sectionNN/item_slug
   ```
...
每个 `.cpp` 在 `learn::topic<...>` 里的 id 形如 `part2/stage02/sectionMM/<item_slug>`,
跟文件路径一一对应, 例:
```
part2/stage02/section01/<item_slug>
```
