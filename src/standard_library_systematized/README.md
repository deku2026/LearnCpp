...
```bash
ls src/standard_library_systematized
```
...
4. 跑这一阶段: `ctest --preset windows-debug --output-on-failure -R 'standard_library_systematized__'`;
...
ctest --preset windows-debug --output-on-failure -R 'standard_library_systematized__'
```
...
- 文件名 -> 新的"小项" slug;
- 顶部 `Item :` 注释;
- `TEST(standard_library_systematized__<topic>, <new_item>)` 里的 `<new_item>`.
