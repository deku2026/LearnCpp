// LearnCpp placeholder
// Doc     : part6-branch-c-memory-management.md
// Stage   : part6_branch_c_memory_management
// Section : section06_allocator_model
// Item    : forbidden_user_specialization_of_allocator_traits_cpp23
//
// TODO: read cppreference, sketch a minimal example, check godbolt / C++ Insights,
//       then replace GTEST_SKIP with real EXPECT_* / ASSERT_* assertions.

#include <gtest/gtest.h>

namespace learn_cpp::part6_branch_c::section06 {

TEST(part6_branch_c_section06, forbidden_user_specialization_of_allocator_traits_cpp23) {
    GTEST_SKIP() << "todo: forbidden_user_specialization_of_allocator_traits_cpp23";
}

}  // namespace learn_cpp::part6_branch_c::section06
